...
import { ContainerAppEnvironmentCertificate } from '@cdktf/provider-azurerm/lib/container-app-environment-certificate';
...
export class ContainerAppEnvConstruct extends Construct {
  ...
  constructor(scope: Construct, name: string, config: ContainerAppEnvConfig) {
    ...
    let certificate: ContainerAppEnvironmentCertificate | undefined;
    if (config.certificate) {
      certificate = new ContainerAppEnvironmentCertificate(this, 'certificate', {
        name: 'difycerts',
        containerAppEnvironmentId: this.environment.id,
        certificateBlobBase64: `\${filebase64("${config.certificate.path}")}`,
    ...
    this.nginxApp = new ContainerApp(this, 'nginx', {
      ...
      ingress: {
        ...
        ...(config.certificate && {
          customDomain: [
            {
              name: config.certificate.customerDomain,
    ...
    new ContainerApp(this, 'worker', this.createWorkerAppConfig(config));

    // API Container App
    new ContainerApp(this, 'api', this.createApiAppConfig(config));
   ...
   */
  private createWorkerApiEnvVars(config: ContainerAppEnvConfig, isApi: boolean = false): Array<{ name: string; value: string }> {
    /* base env vars
    ...
    */
    const baseEnvVars = [
      ...
      { name: 'DB_PASSWORD', value: config.postgresConfig.adminPassword },
      ...
      { name: 'PGVECTOR_PASSWORD', value: config.postgresConfig.adminPassword },
